...
public class CameraFragment extends Fragment implements ActivityCompat.OnRequestPermissionsResultCallback {
    ...
    private Semaphore mCameraOpenCloseLock = new Semaphore(1); // TODO: 19.10.17 This 'Semaphore thingie seems cool. And not only because #OpenTTD (-.o)
    ...
    private final View.OnClickListener mClickListener = view -> {
        switch (view.getId()) {
            ...
            case R.id.info: {
                EpilepticaActivity.start(getActivity());
                break;
            }
        }
    };
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        if (requestCode == REQUEST_CAMERA_PERMISSION) {
            if (grantResults.length != 1 || grantResults[0] != PackageManager.PERMISSION_GRANTED) {
                ErrorDialog.newInstance("Can I haz purrrmissionz plz? ก₍⸍⸌̣ʷ̣̫⸍̣⸌₎ค").show(getChildFragmentManager(), this.getClass().getName());
            }
        } else {
    ...
    private void setUpCameraOutputs(int width, int height) {
        ...
        try {
            for (String cameraId : manager.getCameraIdList()) {
                ...
                Size largest = Collections.max(Arrays.asList(map.getOutputSizes(ImageFormat.JPEG)), new SizeAreaComparator());
...
//                }
    ...
    /** Instantiate and start a background thread and its {@link Handler}. */
    ...
    /** Stops and release the background thread and its {@link Handler}. */
     ...
     * we get a response in {@link #mCaptureCallback} from {@link #captureStillPicture()}.
     */
     ...
     * {@link #mCaptureCallback} from both {@link #captureStillPicture()}.
     */
    ...
    static class SizeAreaComparator implements Comparator<Size> {
